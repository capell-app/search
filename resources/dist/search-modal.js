// Generated from resources/js/search-modal.js. Run npm run build.
;(() => {
  // resources/js/search-modal.js
  ;(() => {
    ...
    const typeIcon = (type) => {
      ...
      return (
        '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="h-4 w-4"><path stroke-linecap="round" stroke-linejoin="round" d="' +
        path +
        '" /></svg>'
      )
    }
    ...
    const stateByDialog = /* @__PURE__ */ new WeakMap()
    ...
    const closeDialog = (dialog, { restoreFocus = true } = {}) => {
      ...
      document
        .querySelectorAll(`${selectors.trigger}[aria-controls="${dialog.id}"]`)
        .forEach((trigger) => {
    ...
    const setLoading = (dialog, query) => {
      ...
      if (loading) {
        const label =
          ...
          'Searching for ":query"'
        loading.querySelector('[data-site-search-loading-label]').textContent =
      ...
      input.removeAttribute('aria-activedescendant')
      status.textContent =
        loading?.getAttribute('data-site-search-loading-status') || 'Searching'
    }
    ...
    const renderResults = (dialog, payload) => {
      ...
      const suggestionsTemplate =
        resultsRegion?.getAttribute('data-site-search-suggestions-template') ||
      ...
      if (empty) {
        ...
        if (!hasItems) {
          const label = empty.querySelector('[data-site-search-empty-label]')
          const template =
      ...
      allResults.href = payload.allResultsUrl || input.form?.action || '/search'
